...
export class CarsComponent {
  ...
  checkedFilter(): CarsFilter {
    const checkedFilter: CarsFilter = {
      makers: this.filter.makers
        .filter((item) => item.checked)
        .map((item) => item),
      bodyTypes: this.filter.bodyTypes
        .filter((item) => item.checked)
        .map((item) => item),
      powerTrains: this.filter.powerTrains
        .filter((item) => item.checked)
        .map((item) => item),
      driveSystems: this.filter.driveSystems
        .filter((item) => item.checked)
        .map((item) => item),
    };
  ...
  onCloseMakerTag(targetValue: string) {
    const itemToUpdate = this.filter.makers.find(
      (i) => i.value === targetValue
    );
    if (itemToUpdate) {
  ...
  onCloseBodyTypeTag(targetValue: string) {
    const itemToUpdate = this.filter.bodyTypes.find(
      (i) => i.value === targetValue
    );
    if (itemToUpdate) {
      itemToUpdate.checked = false;
  ...
  onClosePowerTrainTag(targetValue: string) {
    const itemToUpdate = this.filter.powerTrains.find(
      (i) => i.value === targetValue
    );
    if (itemToUpdate) {
      itemToUpdate.checked = false;
  ...
  onCloseDriveSystemTag(targetValue: string) {
    const itemToUpdate = this.filter.driveSystems.find(
      (i) => i.value === targetValue
    );
    if (itemToUpdate) {
      itemToUpdate.checked = false;
