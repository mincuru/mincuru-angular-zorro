...
describe('CarsComponent', () => {
  ...
  it("checkedFilter() should return checked items", () => {
    ...
    expect(component.checkedFilter().makers).toContain({ title: 'ホンダ', value: 'ホンダ', checked: true  });
    expect(component.checkedFilter().bodyTypes).toContain({ title: 'Sedan', value: 'SEDAN', checked: true  });
    expect(component.checkedFilter().powerTrains).toContain({ title: 'マイルドHV', value: 'MldHV', checked: true  });
    expect(component.checkedFilter().driveSystems).toContain({ title: 'FR', value: 'FR', checked: true  });
  });
  ...
  it("onCloseMakerTag() should update checked value", () => {
    ...
    expect(component.filter.makers).toContain({ title: '日産', value: '日産', checked: false  });
  });
});
